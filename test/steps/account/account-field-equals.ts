...
describe('AccountFieldEquals', () => {
  ...
  describe('Metadata', () => {
    it('should return expected step metadata', () => {
      ...
      expect(stepDef.getExpression()).to.equal('the (?<field>[ a-zA-Z0-9_-]+) field on salesloft account (?<name>.+) from (?<domain>.+) domain should (?<operator>be set|not be set|be less than|be greater than|be one of|be|contain|not be one of|not be|not contain) ?(?<expectation>.+)?');
