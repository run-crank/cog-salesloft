...
```shell-session
$ crank cog:install stackmoxie/salesloft
```
...
### Authentication
<!-- run `crank cog:readme stackmoxie/salesloft` to update -->
...
$ crank cog:auth stackmoxie/salesloft
```
...
| **Check a field on a SalesLoft Person**<br>(`PersonFieldEqualsStep`) | `the (?<field>[ a-zA-Z0-9_-]+) field on salesloft person (?<email>.+) should (?<operator>be set\|not be set\|be less than\|be greater than\|be one of\|be\|contain\|not be one of\|not be\|not contain) ?(?<expectation>.+)?` | - `email`: Person's email address <br><br>- `field`: Field name to check <br><br>- `operator`: Check Logic (be, not be, contain, not contain, be greater than, be less than, be set, not be set, be one of, or not be one of) <br><br>- `expectation`: Expected field value |
<!-- stepDetailsEnd -->
...
5. Run `crank cog:install --source=local --local-start-command="npm start"` to
   ...
   flag or run `crank cog:uninstall stackmoxie/salesloft` if you've already
   installed the distributed version of this Cog.

### Adding/Modifying Steps
Modify code in `src/steps` and validate your changes by running
`crank cog:step stackmoxie/salesloft` and selecting your step.
...
re-authenticate this Cog by running `crank cog:auth stackmoxie/salesloft`

### Tests and Housekeeping
Tests can be found in the `test` directory and run like this: `npm test`.
Ensure your code meets standards by running `npm run lint`.

[what-is-crank]: https://crank.run?utm_medium=readme&utm_source=stackmoxie%2Fsalesloft
